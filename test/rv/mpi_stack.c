...
//extern int n_iter_a_stack;

extern int N_parm;
extern int N_swap;
extern int Swapmode;
extern int SameBetaCopy;
extern int SameBC_start;
extern int SameBC_ratio;

extern int RestoreTrueTune;

extern int SameBetaSwap;
...
int save_debug_stack_doswap(int i_swap, int j_swap);
...
int mpi_judge_and_swap(MPI_Status status, int my_rank, int root_rank, int slave_rank, int rootsent_tag, int slavereturn_tag, double **transit_BetaParm_root, int nline_data, double *data_NlineNdim, double *logpost_all_ranks, int i_swap, int j_swap, int Swapmode, int n_ranks, double *running_Beta_Values, unsigned *ptr_i_accumul);

// swap the values of two chain
void swap_two_chains(double *ptr_ichain, double *ptr_jchain, int N_parm);
void chain1_mirror_chain2(double *ptr_ichain, double *ptr_jchain, int N_parm);
...
{    
    ...
    while ( i_next < n_iter_a_stack )
    {
        ...
        //
	if ( ((int)*ptr_i_accumul<RestoreTrueTune) )  // start of the "if judgements" that turn off the swap after N > RestoreTrueTune
	{
            for (int i=0; i<N_swap; i++)
            {
                ...
                //
    	    // do not judge and swap at the last iter
                if ( ((int)*ptr_i_accumul % n_iter_a_stack) != 0) 
    	    {
                    // Judge and make swap in case needed
                    do_swap = mpi_judge_and_swap(status, my_rank, root_rank, slave_rank, rootsent_tag, slavereturn_tag, transit_BetaParm_root, nline_data, data_NlineNdim, logpost_all_ranks, i_swap, j_swap, Swapmode, n_ranks, running_Beta_Values, ptr_i_accumul);
                }
                //
                //
                // adjust Ladders if T_L == 1
        ...
        //
	}  // end of the "if judgements" that turn off the swap after N > RestoreTrueTune
        //
        // sync since we need to pass logpost to the ranks in mpi_batch
        MPI_Barrier(MPI_COMM_WORLD);
        //
...
{    
    ...
    if( my_rank == root_rank )
    {
        ...
        {
	    // same beta always swap because exp(0) = 1
	...
	//
        if (do_swap) 
        {
	    if ( (SameBetaCopy) && ( ((int)*ptr_i_accumul>SameBC_start) && ((int)*ptr_i_accumul<RestoreTrueTune) ) )
	    {
	        // NOTE: we assume all logpost are negative
                if ( (logpost_ichain_ibeta>logpost_jchain_jbeta) && (logpost_jchain_jbeta/logpost_ichain_ibeta>SameBC_ratio) )
                {
                    chain1_mirror_chain2(ptr_jchain, ptr_ichain, N_parm);
                    logpost_all_ranks[j_swap] = logpost_ichain_ibeta;
	        }
	        else if ( (logpost_jchain_jbeta>logpost_ichain_ibeta) && (logpost_ichain_ibeta/logpost_jchain_jbeta>SameBC_ratio) )
                {
                    chain1_mirror_chain2(ptr_ichain, ptr_jchain, N_parm);
                    logpost_all_ranks[i_swap] = logpost_jchain_jbeta;
                }
	    }
        ...
        if (do_swap)
        {
	    if ( (running_Beta_Values[j_swap]!=running_Beta_Values[i_swap]) || (SameBetaSwap && ((int)*ptr_i_accumul<RestoreTrueTune) ) )
	    {
		...
		//
                if (save_swap_debug)
                {
                    save_debug_stack_doswap(i_swap, j_swap);
                }
		//
	    }
       }
