        ...
        if ( (i_next_stack > N_begintune) &&  (i_next_stack < N_stoptune) && (i_next_stack < N_iter) )
        {
            ...
            for (int i_rank=0; i_rank<n_ranks; i_rank++)
            {
                if (tune_ranks[i_rank] == 1)
                {
                    ...
                    mpi_tune_sigma_irank(status, my_rank, n_ranks, root_rank, rootsent_tag, slavereturn_tag, transit_BetaParm_root, logpost_all_ranks, nline_data, data_NlineNdim, ptr_sigma_prop, n_iter_in_tune, i_rank, sigma_RanksParm_root, running_Beta_Values, ptr_i_accumul);
